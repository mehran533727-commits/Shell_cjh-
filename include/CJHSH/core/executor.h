#ifndef CJHSH_CORE_EXECUTOR_H
#define CJHSH_CORE_EXECUTOR_H
...
// live in CJHSH/shell.h since the state struct needs to see them too.
//
// Split out of the old mega-header CJHSH/core.h. `CJHSH/core.h` still
...
void background_process(const std::vector<std::string> &argv,
                        ShellState &state,
                        const std::vector<Redirection> &redirections);
void background_command(const std::vector<std::string> &argv,
                        ShellState &state,
                        const std::vector<Redirection> &redirections);
int register_background_job(pid_t pid, const std::string &command,
                            ShellState &state);
void check_background_process_finished(
    std::unordered_map<pid_t, CoreState::BackgroundJob> &background_processes);
