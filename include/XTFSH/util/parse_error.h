#ifndef XTFSH_UTIL_PARSE_ERROR_H
#define XTFSH_UTIL_PARSE_ERROR_H

// Lightweight parse-error reporting. Previously parser/executor emitted
// bare "XTFSH: unmatched '('" strings with no indication of where in the
...
//   * emit_parse_error — routes through XTFSH::io::error with the
//                       "XTFSH:LINE:COL: error: MSG" format shells like
...
// Emit `err` via XTFSH::io::error with the canonical
// "XTFSH:LINE:COL: error: MSG" prefix. Column 0 is elided.
void emit_parse_error(const ParseError &err);

} // namespace XTFSH::parse

#endif // XTFSH_UTIL_PARSE_ERROR_H
