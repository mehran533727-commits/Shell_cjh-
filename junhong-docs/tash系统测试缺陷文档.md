# tash（Shell_cjh）系统测试缺陷文档

> 测试对象：Shell_cjh-（项目本名 tash，Tavakkoli's Shell），C++17 Linux shell，v2.2.0
...
- **现象**：`@ai <解释类问题>` 在管道（非 TTY stdout）下有时只显示 `tash ai ─` 前缀、正文空白；在真实终端（PTY）下正常显示。
