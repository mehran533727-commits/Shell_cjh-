# XTFSH（Shell_cjh）功能文档

> 项目：Shell_cjh-（本名 XTFSH，XTF'SH Shell），C++17 Linux shell，v2.2.0
...
XTFSH 是一个用 C++17 编写的现代 Unix shell（命令解释器）。它读取用户输入的一行命令，经过解析、展开、分发，最终通过 Linux 进程 API（`fork` / `execvp` / `waitpid`）执行外部程序或内建命令。除完整覆盖课程要求的 shell 基础功能外，还内置了 AI 自然语言助手、语法高亮、Tab 补全、主题、SQLite 历史等增强特性。
...
```
产物：`build/XTFSH.out`（约 2.2 MB）

### 运行
```bash
./build/XTFSH.out              # 交互模式（显示 XTFSH banner）
./build/XTFSH.out demo.XTFSH    # 脚本模式
./build/XTFSH.out --version    # 版本与已编入特性
```
...
> 关于 ls/cat/grep/echo：这些是系统外部命令，任何 shell 都通过 `fork`+`exec` 调用它们，非 shell 自身实现。XTFSH 的 `echo` 也走外部 `/usr/bin/echo`。
...
**交互体验**：语法高亮 · fish 风格灰色历史提示 · 快捷键（→ 接受提示 / Alt+→ 接受一词 / Alt+. 上条末参 / Ctrl-L 清屏）· 两行/starship prompt · XTFSH banner
...
- 配置文件：`~/.config/XTFSH/ai_provider`（=openai）、`~/.config/XTFSH/openai_key`（DeepSeek key，0600）
...
| `include/XTFSH/shell.h` | 核心数据结构：`ShellState`、`CommandSegment`、`Redirection` 等 |
...
- **未由自动化覆盖的**：真实终端的交互观感（banner、配色、Tab 补全弹窗、灰色提示），需在终端 `./build/XTFSH.out` 亲验。

---

*本文档描述系统客观功能与用法；已知问题与修复策略见《XTFSH系统测试缺陷文档.md》。*
