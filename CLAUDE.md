...
| `-DXTFSH_SANITIZERS=ON` | ASan + UBSan; pair with `-DCMAKE_BUILD_TYPE=RelWithDebInfo` |
| `-DXTFSH_COVERAGE=ON` | `--coverage` instrumentation (GCC/Clang only) |
| `-DXTFSH_STATIC=ON` | `-static` link (really only works on Linux/musl) |
| `-DXTFSH_ENABLE_FUZZER=ON` | Build `XTFSH_parser_fuzzer` (clang required, set `-DCMAKE_CXX_COMPILER=clang++`) |
...
`include/XTFSH/plugin.h` defines four provider interfaces and a priority-based `PluginRegistry`:
...
Each plugin registers itself in `cmake/plugin_list.cmake` — **append one `XTFSH_register_plugin(...)` call at the bottom**, nothing else. That function (defined in `cmake/plugins.cmake`) appends to `SHELL_SOURCES`, records a test spec, and `XTFSH_finalize_plugin_tests()` materializes the `test_<NAME>` binary once `shell_lib` exists. This pattern exists to avoid per-PR merge conflicts on a central source list.

`REQUIRES XTFSH_AI_ENABLED` / `REQUIRES XTFSH_SQLITE_ENABLED` gates a plugin on optional deps. `TEST_STANDALONE` means the test does not link `shell_lib` — use it when the test compiles its own subset of sources. `TEST_AI_AWARE` adds the AI define + libcurl link when AI is on.

### CMake layout

The root `CMakeLists.txt` is thin (~100 lines). The real work is in `cmake/`:

- `features.cmake` — default build type, `find_package()` for libcurl (required), SQLite3 (optional), nlohmann_json (optional; fetched if missing). Sets `XTFSH_AI_ENABLED` / `XTFSH_SQLITE_ENABLED`.
...
- `tests.cmake` — builds `shell_lib` (shared with tests; same sources as `XTFSH.out` minus `src/main.cpp`), then per-plugin test executables via `XTFSH_finalize_plugin_tests()`, then a single `test_integration` binary that globs `tests/integration/test_*.cpp` with `CONFIGURE_DEPENDS`.
- `fuzzer.cmake`, `sanitizers.cmake` — optional toggles.

`data/ai_models.json` is embedded into the binary at build time via `cmake/ai_models_embedded.cpp.in`. The env var `XTFSH_AI_MODELS_JSON` overrides at runtime — useful for tests and ops.

### Tests

~779 tests across 24 unit + 41 integration files. Integration tests use `tests/test_helpers.h` (`run_shell(...)`) which writes input to a tmp file, pipes it into `$XTFSH_SHELL_BIN` (set by CTest to `$<TARGET_FILE:XTFSH.out>`), and captures stdout+stderr. `count_occurrences()` exists because GNU readline echoes input to stdout on Linux when stdin is a file — tests that check "did the command run" need to distinguish echo from real output.
...
- **Parser error format**: all parse errors emit `XTFSH: error: L:C: <msg>` (see commit `55faeba`). Match this exactly when adding new error paths.
- **Diagnostic output**: use `XTFSH::io::debug` for subsystem diagnostics (see commit `d3ea8c4`). Don't write to `stderr` directly in signal/history/bg/plugin/config code.
...
FetchContent tarballs are pre-fetched with curl retries in each job to dodge intermittent codeload 504s; `XTFSH_FC_ARGS` points CMake at the pre-fetched dirs.
