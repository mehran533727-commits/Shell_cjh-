...
// std::atomic<pid_t> live in include/XTFSH/shell.h so every TU that reads
// this atomic from a signal-adjacent path inherits the check.
volatile sig_atomic_t sigchld_received = 0;
std::atomic<pid_t> fg_child_pid{0};

// Pending-trap flags: one slot per signum up to XTFSH_MAX_SIGNAL. The
...
#ifndef XTFSH_MAX_SIGNAL
#define XTFSH_MAX_SIGNAL 64
#endif
volatile sig_atomic_t pending_traps[XTFSH_MAX_SIGNAL] = {0};
...
static void sigint_handler(int) {
    ...
    XTFSH::ai::abort_flag::sigint_raise();
    // Queue the trap; the main loop fires it between commands.
    if (SIGINT < XTFSH_MAX_SIGNAL) pending_traps[SIGINT] = 1;
}
...
static void trap_only_handler(int signum) {
    if (signum >= 0 && signum < XTFSH_MAX_SIGNAL) pending_traps[signum] = 1;
}
...
void reset_child_signal_state() {
    ...
    // Restore default disposition for the handlers XTFSH installs. The
...
void install_trap_handler(int signum) {
    if (signum <= 0 || signum >= XTFSH_MAX_SIGNAL) return;
