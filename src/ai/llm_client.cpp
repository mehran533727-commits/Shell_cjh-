...
// (exposed via XTFSH::ai::diag) so unit tests can exercise the exact
// output format without mocking HTTPS. Aliased into file scope for
// brevity; every failure site below funnels through these.
// ═════════════════════════════════════════════════════════════════

using XTFSH::ai::diag::log_http_failure;
using XTFSH::ai::diag::log_curl_failure;
using XTFSH::ai::diag::log_request_debug;
using XTFSH::ai::diag::log_response_debug;
using XTFSH::ai::diag::truncate_for_debug;
...
static std::size_t XTFSH_max_response_bytes() {
    static const std::size_t cached = []() -> std::size_t {
        const char *env = std::getenv("XTFSH_AI_MAX_RESPONSE_BYTES");
        if (env && *env) {
...
static size_t XTFSH_curl_write_cb(char *ptr, size_t size, size_t nmemb, void *userdata) {
    size_t total = size * nmemb;
    CurlStreamContext *ctx = static_cast<CurlStreamContext*>(userdata);
    ctx->total_bytes += total;
    if (ctx->total_bytes > XTFSH_max_response_bytes()) {
        ctx->overflowed = true;
...
static size_t XTFSH_curl_buffer_cb(char *ptr, size_t size, size_t nmemb, void *userdata) {
    size_t total = size * nmemb;
    auto *ctx = static_cast<CurlBufferContext*>(userdata);
    if (ctx->body.size() + total > XTFSH_max_response_bytes()) {
        ctx->overflowed = true;
        return 0;  // aborts transfer with CURLE_WRITE_ERROR
    }
...
enum class TransportError {
    ...
    Overflow,       // response exceeded XTFSH_AI_MAX_RESPONSE_BYTES
...
static int XTFSH_curl_abort_cb(void *, curl_off_t, curl_off_t, curl_off_t, curl_off_t) {
    return XTFSH::ai::abort_flag::should_abort() ? 1 : 0;
}
...
static bool configure_common_curl_opts(CURL *curl,
    ...
    curl_easy_setopt(curl, CURLOPT_XFERINFOFUNCTION, XTFSH_curl_abort_cb);
    ...
    bool allow_plain_http = false)
{
    ...
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, XTFSH_curl_buffer_cb);
    ...
    bool allow_plain_http = false)
{
    ...
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, XTFSH_curl_write_cb);
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, &ctx);
...
GeminiClient::GeminiClient(const string &api_key)
    : api_key_(api_key),
      ...
      model_(XTFSH::ai::default_model_for("gemini")),
      connect_timeout_(10),
      read_timeout_(30) {
    for (const auto &m : XTFSH::ai::fallback_models_for("gemini")) {
...
OpenAIClient::OpenAIClient(const string &api_key)
    : api_key_(api_key),
      model_(XTFSH::ai::default_model_for("openai")),
      connect_timeout_(10),
      read_timeout_(60) {}
...
// piece to the emit callback. See XTFSH/llm_client.h for rationale.
// ═════════════════════════════════════════════════════════════════
