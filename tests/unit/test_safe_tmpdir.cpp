...
TEST_F(SafeTmpDirFixture, WorldWritableTmpdirIsRejected) {
    created = "/tmp/XTFSH_safe_tmpdir_test_" + std::to_string(::getpid());
    std::error_code ec;
    fs::create_directories(created, ec);
    ::chmod(created.c_str(), 0777);

    ::setenv("TMPDIR", created.c_str(), 1);
    std::string resolved = XTFSH::util::resolve_safe_tmpdir();
...
TEST_F(SafeTmpDirFixture, ProperlyLockedTmpdirIsAccepted) {
    created = "/tmp/XTFSH_safe_tmpdir_trust_" + std::to_string(::getpid());
    std::error_code ec;
    fs::create_directories(created, ec);
...
TEST_F(SafeTmpDirFixture, UnsetTmpdirFallsBackToTmp) {
    ::unsetenv("TMPDIR");
    EXPECT_EQ(XTFSH::util::resolve_safe_tmpdir(), "/tmp");
}

TEST_F(SafeTmpDirFixture, NonExistentTmpdirFallsBackToTmp) {
    ::setenv("TMPDIR", "/tmp/XTFSH_safe_tmpdir_does_not_exist_XYZZY", 1);
    EXPECT_EQ(XTFSH::util::resolve_safe_tmpdir(), "/tmp");
}
