...
using namespace XTFSH::config_sync;
...
protected:
    ...
    void SetUp() override {
        ...
        char tmpl[] = "/tmp/XTFSH_config_sync_test_XXXXXX";
...
TEST_F(ConfigSyncTest, SetRemoteAddsOrigin) {
    ...
    bool result = sync_set_remote(temp_dir, "https://github.com/user/XTFSH-config.git");
    ASSERT_TRUE(result);

    CmdResult remote_out = run_git_argv(temp_dir, {"remote", "-v"});
    EXPECT_EQ(remote_out.exit_code, 0);
    EXPECT_NE(remote_out.output.find("https://github.com/user/XTFSH-config.git"),
              std::string::npos);
}
