...
TEST(Basic, EchoQuotedStripsQuotes) {
    ...
    std::string testfile = "/tmp/XTFSH_quote_test_" + std::to_string(getpid()) + ".txt";
...
TEST(Basic, SigintDoesNotCrashShell) {
    ...
    std::string marker = "/tmp/XTFSH_sigint_" + std::to_string(getpid());
    unlink(marker.c_str());
    // The script sends SIGINT to its own process group, then runs echo
    // to prove the shell survived.
    std::string script = "/tmp/XTFSH_sigint_test_" + std::to_string(getpid()) + ".sh";
    {
