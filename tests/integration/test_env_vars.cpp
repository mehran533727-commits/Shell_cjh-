...
TEST(EnvVars, ExportAndRead) {
    auto r = run_shell("export XTFSH_TEST_VAR=gtest_value_42\necho $XTFSH_TEST_VAR\nexit\n");
...
TEST(EnvVars, UnsetVariable) {
    ...
    // If unset works, $XTFSH_UNSET_FILE expands to empty and the redirect target is empty/fails.
    // Instead, verify by exporting a new value after unset — it should not have the old value.
    std::string marker = "/tmp/XTFSH_unset_" + std::to_string(getpid());
    unlink(marker.c_str());
    run_shell("export XTFSH_UNSET_VAR=old_value\nunset XTFSH_UNSET_VAR\nexport XTFSH_UNSET_VAR=new_value\necho $XTFSH_UNSET_VAR > " + marker + "\nexit\n");
...
TEST(EnvVars, UndefinedVarExpandsEmpty) {
    ...
    auto r = run_shell("export XTFSH_UNDEF_CHECK=found_it\necho $XTFSH_NEVER_DEFINED_XYZ99\necho $XTFSH_UNDEF_CHECK\nexit\n");
...
TEST(EnvVars, DollarQuestionInChainedSemicolon) {
    ...
    std::string marker = "/tmp/XTFSH_chain_" + std::to_string(getpid());
    unlink(marker.c_str());
...
TEST(EnvVars, DollarQuestionCommandNotFound127) {
    std::string marker = "/tmp/XTFSH_127_" + std::to_string(getpid());
    unlink(marker.c_str());
...
TEST(EnvVars, SingleQuotesPreventExpansion) {
    ...
    std::string script = "/tmp/XTFSH_sq_test_" + std::to_string(getpid()) + ".sh";
    std::string marker = "/tmp/XTFSH_sq_out_" + std::to_string(getpid());
    {
        std::ofstream f(script);
        f << "export XTFSH_SQ_TEST=secret\n";
        f << "echo '$XTFSH_SQ_TEST' > " << marker << "\n";
    }
    run_shell_script(script);
    std::string content = read_file(marker);
    EXPECT_NE(content.find("$XTFSH_SQ_TEST"), std::string::npos)
