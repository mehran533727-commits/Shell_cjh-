...
TEST(Redirect, FileCreatedWithCorrectPermissions) {
    std::string testfile = "/tmp/XTFSH_gtest_perms_" + std::to_string(getpid()) + ".txt";
...
TEST(Redirect, OverwriteExistingFile) {
    std::string testfile = "/tmp/XTFSH_gtest_overwrite_" + std::to_string(getpid()) + ".txt";
...
TEST(Redirect, AppendToFile) {
    std::string testfile = "/tmp/XTFSH_gtest_append_" + std::to_string(getpid()) + ".txt";
...
TEST(Redirect, InputFromFile) {
    std::string infile = "/tmp/XTFSH_gtest_input_" + std::to_string(getpid()) + ".txt";
...
TEST(Redirect, AppendDoesNotOverwrite) {
    std::string testfile = "/tmp/XTFSH_gtest_append2_" + std::to_string(getpid()) + ".txt";
...
TEST(Redirect, StderrToFile) {
    std::string errfile = "/tmp/XTFSH_gtest_stderr_" + std::to_string(getpid()) + ".txt";
    // ls on a nonexistent path writes an error to stderr
    run_shell("ls /nonexistent_path_for_XTFSH_test 2> " + errfile + "\nexit\n");
...
TEST(Redirect, StderrToStdout) {
    ...
    auto r = run_shell("ls /nonexistent_path_for_XTFSH_test 2>&1\nexit\n");
...
TEST(Redirect, StderrToFileDoesNotAffectStdout) {
    std::string outfile = "/tmp/XTFSH_gtest_stdout_" + std::to_string(getpid()) + ".txt";
    std::string errfile = "/tmp/XTFSH_gtest_stderr2_" + std::to_string(getpid()) + ".txt";
