...
// XTFSH_AUTO_LINKIFY=1 makes every external command's stdout get its URLs
...
TEST(RichOutputIntegration, AutoLinkifyWrapsExternalCommandOutput) {
    setenv("XTFSH_AUTO_LINKIFY", "1", 1);
    std::string path = "/tmp/XTFSH_autolinkify_" + std::to_string(getpid());
    {
        std::ofstream f(path);
        f << "visit https://example.com today\n";
    }
    auto r = run_shell("cat " + path + "\nexit\n");
    unsetenv("XTFSH_AUTO_LINKIFY");
    unlink(path.c_str());
    EXPECT_NE(r.output.find("\x1b]8;;https://example.com\x1b\\"),
              std::string::npos);
}
...
TEST(RichOutputIntegration, NoAutoLinkifyByDefault) {
    unsetenv("XTFSH_AUTO_LINKIFY");
    std::string path = "/tmp/XTFSH_autolinkify_off_" + std::to_string(getpid());
    {
        std::ofstream f(path);
        f << "visit https://example.com today\n";
    }
...
TEST(RichOutputIntegration, AutoLinkifyBypassedByRedirection) {
    setenv("XTFSH_AUTO_LINKIFY", "1", 1);
    std::string in = "/tmp/XTFSH_redir_in_" + std::to_string(getpid());
    std::string out = "/tmp/XTFSH_redir_out_" + std::to_string(getpid());
    {
        std::ofstream f(in);
