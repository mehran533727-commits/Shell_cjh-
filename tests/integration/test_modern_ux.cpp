...
TEST(Suggest, ExitCode127OnCommandNotFound) {
    std::string marker = "/tmp/XTFSH_suggest_exit_" + std::to_string(getpid());
    unlink(marker.c_str());
...
TEST(AutoCd, DirectoryNameChangesDir) {
    ...
    std::string script = "/tmp/XTFSH_autocd_s_" + std::to_string(getpid()) + ".sh";
    {
...
TEST(AutoCd, TildeExpandsAndChangesDir) {
    std::string script = "/tmp/XTFSH_autocd_tilde_" + std::to_string(getpid()) + ".sh";
    {
        std::ofstream f(script);
        f << "~\n";
    }
...
TEST(Multiline, ScriptBackslashContinuation) {
    std::string script = "/tmp/XTFSH_ml_" + std::to_string(getpid()) + ".sh";
    std::string marker = "/tmp/XTFSH_ml_out_" + std::to_string(getpid());
    {
        std::ofstream f(script);
        f << "echo hello \\\n";
        f << "world > " << marker << "\n";
...
TEST(PersistentHistory, HistoryFileCreated) {
    // Run a command, then check that ~/.XTFSH_history exists
...
TEST(ZCommand, ZAfterCdFindsDirectory) {
    ...
    std::string script = "/tmp/XTFSH_z_s_" + std::to_string(getpid()) + ".sh";
    {
        std::ofstream f(script);
        f << "cd /tmp\n";
        f << "cd /\n";
...
TEST(HistoryIgnoreSpace, SpacePrefixedNotInHistory) {
    ...
    std::string script = "/tmp/XTFSH_space_" + std::to_string(getpid()) + ".sh";
    {
        std::ofstream f(script);
        f << " echo secret_cmd\n";
        f << "echo visible_cmd\n";
