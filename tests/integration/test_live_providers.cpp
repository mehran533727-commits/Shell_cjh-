...
// registered and actually fire in a running XTFSH process.
...
struct HomeGuard {
    ...
    HomeGuard() {
        ...
        original = h ? h : "";
        tmp = "/tmp/XTFSH_live_providers_" + std::to_string(getpid());
...
// ─────────────────────────────────────────────────────────────────────
// SqliteHistoryProvider: every executed command lands in ~/.XTFSH/history.db
// ─────────────────────────────────────────────────────────────────────
...
TEST(LiveProviders, SqliteHistoryPersistsCommands) {
    ...
    std::string db = home.tmp + "/.XTFSH/history.db";
    struct stat st;
    ASSERT_EQ(stat(db.c_str(), &st), 0)
        << "~/.XTFSH/history.db not created — SqliteHistoryProvider "
